...
export const TerpeneSelectorItem = forwardRef(
  ({terp, disabled, onClick, showDetails, ...props}, ref) => (
    <li>
      <button
        {...props}
        className={clsx(
      ...
      >
        <p
          className="text-xl leading-tight capitalize"
          style={{color: terp.color}}
        >
          {terp.name}
        </p>
        {showDetails && terp.scents ? <p>Scents: {terp.scents}</p> : undefined}
        {showDetails && terp.properties && terp.properties.length ? (
          <p className="leading-tight">
            Properties: {terp.properties.join(', ')}
          </p>
        ) : undefined}
      </button>
    </li>
