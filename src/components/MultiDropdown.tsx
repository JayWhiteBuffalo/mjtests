...
const MultiDropdownButton = forwardRef(function MultiDropdownButton(props, ref) {
  const {placeholder, values, className, ...rest} = props
  ...
  return (
    <Button
      {...rest}
      className={`MultiDropdown ${className}`}
      color="light"
      ref={ref}>
...
const Items = forwardRef(({className, items, values, getItemProps, listRef, activeIndex, onChange, ...rest}, ref) => 
  <ul 
    {...rest}
    className={`MultiDropdownItems ${className || ''}`}
    ref={ref}>
    {items.map((item, index) =>
      <li key={item.key}>
        <button 
          ...
          className={`MultiDropdownItem ${values[item.key] ? 'selected' : ''}`}
...
export const SmallMultiDropdown = (props: MultiDropdownProps) => {
...
const MultiDropdownSelectedPane = ({id, values, TypeaheadStore, onChange, onClose}: MultiDropdownProps) => {
  const selectedItems = ArrayUtil.sortBy(Object.keys(values).map(TypeaheadStore.getByName), x => x.name)
...
export const LargeMultiDropdown = (props: MultiDropdownProps) => {
  const [expanded, setExpanded] = useState(false)
  const {refs, floatingStyles, context} = useFloating({
    open: expanded,
    onOpenChange: setExpanded,
