...
export const VendorSchedule = {
  ...
  readSchedule(spec) {
    const week = VendorSchedule.daysOfWeek.map(
      ({abbr}) =>
  ...
  getNamedDay(day) {
    ...
    if (month === 2 && dayOfWeek === 1 && nthDayOfMonth === 2)
      return 'presidents'
    if (month === 5 && dayOfWeek === 1 && 31 - 7 <= dayOfMonth)
      return 'memorial'
    ...
    if (month === 10 && dayOfWeek === 1 && nthDayOfMonth === 1)
      return 'columbus'
    if (day.endsWith('-11-11')) return 'veterans'
    if (month === 11 && dayOfWeek === 4 && nthDayOfMonth === 3)
      return 'thanksgiving'
  ...
  getStatus(schedule) {
    ...
    const yesterdaySchedule = VendorSchedule.getDay(
      DateUtil.addYmd(today, -1),
      schedule,
    )
    const todaySchedule = VendorSchedule.getDay(today, schedule)
    const tomorrowSchedule = VendorSchedule.getDay(
      DateUtil.addYmd(today, 1),
      schedule,
    )
    ...
    if (yesterdaySchedule instanceof Array) {
      if (
        yesterdaySchedule[0] <= timeOfDay + 24 * 3600 &&
        timeOfDay + 24 * 3600 < yesterdaySchedule[1]
      ) {
        return {
          isOpen: true,
    ...
    if (todaySchedule instanceof Array) {
      if (todaySchedule[0] <= timeOfDay && timeOfDay < todaySchedule[1]) {
        const abuts =
          todaySchedule[1] === 24 * 3600 &&
          tomorrowSchedule instanceof Array &&
          tomorrowSchedule[0] === 0
        return {
          isOpen: true,
