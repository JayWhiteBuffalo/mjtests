...
export const PhoneLink = ({tel}) =>
  <BlueExternalLink href={`tel:${tel}`} newTab={false} className="VendorPopupPhone inline-flex items-center">
    <MdPhone className="mr-1" />{formatPhone(tel)}
  </BlueExternalLink>

const VendorContact = ({contact}) =>
  <div className="flex justify-between">
    {contact.url
      ? <VendorWebsite url={contact.url} />
      : undefined
    }
    {contact.tel
      ? <PhoneLink tel={contact.tel} />
      : undefined
    }
  </div>
  ...
  if (status.closingSoon) {
    return (
      <span className="ProductVendorOpenStatus">
        ...
        &nbsp;•&nbsp;
        <span className="text-amber-600">Closing soon at {DateUtil.formatShortAmPm(status.closes)}</span>
      </span>
    )
