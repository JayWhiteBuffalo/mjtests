...
const Page = async ({ searchParams: query }) => {
  ...
  // Fetch products and related data in parallel
  const [filteredProducts, user] = await Promise.all([
    ProductDto.getProducts(filter),          // Fetch filtered products
    //ProductDto.getProducts(),                // Fetch all products
  //  ProductDto.getProducerProducts(filter),  // Fetch producer products
    UserDto.getCurrent(),                    // Fetch current user
  ])

  // Extract unique brands, stores, and strains
